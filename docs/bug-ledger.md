...
7. **Order-firing tool hard-gated?** The order tool (`order_ready`) must NEVER fire without non-empty, caller-collected modalidad + nombre + teléfono (+ dirección + cash_amount if delivery) and an explicit spoken read-back + "sí" — a provider `required` flag passes empty strings, so enforce presence in the prompt (K5). Offer only categories the tool returned with items (K6).
